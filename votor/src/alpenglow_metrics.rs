...
impl Default for NodeVoteMetrics {
    fn default() -> Self {
        ...
        let histogram = Histogram::new(10, 20).unwrap();
        let metric = Metric {
            count: 0,
            histogram,
        };
...
impl AgMetrics {
    ...
    pub fn record_block_hash_seen(
        ...
        let histogram = self
            .leader_metrics
            .entry(leader)
            .or_insert(Histogram::new(10, 20).unwrap());
    ...
    /// Perform end of epoch reporting and reset all the statistics for the subsequent epoch.
