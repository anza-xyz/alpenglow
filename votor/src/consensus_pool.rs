use {
    crate::{
        ...
        common::{certificate_limits_and_vote_types, vote_to_cert_types, Stake},
        consensus_pool::{
            parent_ready_tracker::ParentReadyTracker, slot_stake_counters::SlotStakeCounters,
        ...
        for cert_type in vote_to_cert_types(vote) {
            ...
            let accumulated_stake = vote_types
                ...
                .map(|vote_type| {
                    self.vote_pools
                        .get(&slot)
                        .map_or(0, |p| p.get_stake(*vote_type, block_id))
                })
                .sum::<Stake>();
            ...
            for vote_type in vote_types {
                if let Some(vote_pool) = self.vote_pools.get(&slot) {
                    cert_builder
                        .aggregate(&vote_pool.get_votes(*vote_type, block_id))
                        .unwrap();
                }
            }
