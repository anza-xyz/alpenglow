        ...
        for cert_id in vote_to_certificate_ids(vote) {
            ...
            vote_types.iter().for_each(|vote_type| {
                ...
                match vote_pool {
                    ...
                    VotePoolType::DuplicateBlockVotePool(pool) => pool.add_to_certificate(voted_block_key.as_ref().expect("Duplicate block pool for {vote_type:?} expects a voted block key for certificate {cert_id:?}"), &mut vote_certificate_builder),
                };
            }
            });
