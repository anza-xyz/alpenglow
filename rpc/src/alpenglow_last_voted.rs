...
impl AlpenglowLastVoted {
    pub fn new() -> Self {
        Self {
            last_voted_map: RwLock::new(HashMap::new()),
        }
    }
    ...
    pub fn get_last_voted(&self, pubkey: &Pubkey) -> Option<Slot> {
        self.last_voted_map.read().unwrap().get(pubkey).map(|(slot, _)| *slot)
    }
}
...
mod tests {
    ...
    fn test_alpenglow_last_voted() {
        let alpenglow_last_voted = AlpenglowLastVoted::new();
        ...
        assert_eq!(alpenglow_last_voted.get_last_voted(&Pubkey::new_unique()), None);
    }
}