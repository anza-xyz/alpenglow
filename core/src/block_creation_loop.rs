...
fn record_and_complete_block(
    ...
    loop {
        ...
        // trace!(
        //     "!!!!! {} :: slot = {} RECORD_RECEIVER WAITING FOR REMAINING SLOT TIME {:?}",
        //     ctx.poh_recorder
        //         .read()
        //         .unwrap()
        //         .bank()
        ...
        let Ok(record) = ctx.record_receiver.try_recv() else {
            // trace!(
            //     "!!!!! {} :: RECORD_RECEIVER NOTHING OBSERVED THIS ITERATION {}",
            //     ctx.poh_recorder
            //         .read()
            //         .unwrap()
            //         .bank()
...
fn sad_leader_handover(
    ...
    start_leader_retry_replay(slot, new_parent_slot, Some(skip_timer), ctx)
}
