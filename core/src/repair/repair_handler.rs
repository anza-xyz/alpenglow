...
mod tests {
    ...
    fn setup_blockstore_with_complete_slot(
        ...
        // Trigger DoubleMerkleMeta computation by requesting the double merkle root
        let block_id = blockstore
            .get_double_merkle_root(slot, BlockLocation::Original)
            .expect("Should be able to compute double merkle root");
