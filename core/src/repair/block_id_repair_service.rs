            ...
            match request {
                ...
                RepairRequest::Shred(shred_request) => {
                    let (addr, bytes) = state
                        .serve_repair
                        .repair_request(
                            repair_info.cluster_slots.as_ref(),
                            shred_request,
                            &mut state.peers_cache,
...
mod tests {
    ...
    fn build_merkle_tree(leaves: &[Hash]) -> (Hash, Vec<Vec<u8>>) {
        ...
        while levels.last().unwrap().len() > 1 {
            ...
            let next_level = (0..level.len())
                .step_by(2)
                .map(|i| {
                    let left = level[i];
                    let right = level[(i + 1).min(level.len() - 1)];
                    join_nodes(left, right)
                })
                .collect();
        ...
        let proofs = (0..leaves.len())
            .map(|leaf_index| {
                ...
                for level in levels.iter().take(levels.len() - 1) {
                    let sibling_index = (index ^ 1).min(level.len() - 1);
                    proof.extend_from_slice(
                        &level[sibling_index].as_ref()[..SIZE_OF_MERKLE_PROOF_ENTRY],
                    );
                    index >>= 1;
                }
                proof
