...
use {
    ...
    crate::repair::{
        ...
        serve_repair::{BlockIdRepairResponse, BlockIdRepairType, RepairRequestProtocol},
    },
...
impl BlockIdRepairService {
        ...
        Builder::new()
            ...
            .spawn(move || {
                ...
                while !exit.load(Ordering::Relaxed) {
                    ...
                    Self::process_responses(&response_receiver, &mut state, &mut throttle);
                    ...
                    for event in std::mem::take(&mut state.pending_repair_events) {
    ...
    fn deserialize_response_and_nonce(packet: PacketRef) -> Option<(BlockIdRepairResponse, u32)> {
        ...
        let response: BlockIdRepairResponse = deserialize_from_with_limit(&mut cursor).ok()?;
        let nonce: u32 = deserialize_from_with_limit(&mut cursor).ok()?;

        cursor.bytes().next().is_none().then_some((response, nonce))
    }
    ...
    fn process_repair_event(
        ...
        if event.slot() <= sharable_banks.root().slot() {
            return;
        }
        ...
        match event {
            RepairEvent::FetchBlock { slot, block_id } => {
                if state.requested_blocks.contains(&(slot, block_id)) {
                    // We've already requested this block, no need to re-request
                    return;
                }

                // Check if we already have the block
                if let Some(location) = blockstore.get_block_location(slot, block_id) {
                    // We have the block, fetch the parent
                ...
                if blockstore.is_dead(slot) {
                    // Turbine block was marked dead, however we're required to fetch the block.
                    // In this context this can only mean one thing - leader equivocated and we received mixed shreds
                ...
                match blockstore.get_double_merkle_root(slot, BlockLocation::Original) {
                    None => {
                        // Turbine/Eager repair hasn't completed yet, defer this event
                        debug_assert!(blockstore.meta(slot).unwrap().is_none_or(|s| !s.is_full()));
                        state
                            .pending_repair_events
                            .push(RepairEvent::FetchBlock { slot, block_id });
                    }
                    Some(turbine_block_id) if turbine_block_id != block_id => {
                        // Turbine ingested a different block, kick off repair for the requested block
                        state.pending_repair_requests.push(RepairRequest::Metadata(
                            BlockIdRepairType::ParentAndFecSetCount { slot, block_id },
                        ));
                        state.requested_blocks.insert((slot, block_id));
                    ...
                    Some(_) => {
                        // Between the blockstore check above and here,
                        // the block was correctly ingested by Turbine/Eager repair, fetch the parent if necessary
    ...
    fn send_requests(
        ...
        let max_batch_len = state
            .pending_repair_requests
            .len()
            .min(MAX_REPAIR_REQUESTS_PER_ITERATION);
