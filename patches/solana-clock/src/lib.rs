...
// At 160 ticks/s, 32 ticks per slot implies that leader rotation and voting will happen
// every 200 ms. A fast voting cadence ensures faster finality and convergence
...
// 1 Dev Epoch = 200 ms * 8192 ~= 27 minutes
...
#[cfg(test)]
static_assertions::const_assert_eq!(DEFAULT_SLOTS_PER_EPOCH, 864_000);
...
#[cfg(test)]
static_assertions::const_assert_eq!(DEFAULT_MS_PER_SLOT, 200);
/// The expected duration of a slot (200 milliseconds).
...
#[cfg(test)]
static_assertions::const_assert_eq!(MAX_RECENT_BLOCKHASHES, 600);
...
#[cfg(test)]
static_assertions::const_assert_eq!(MAX_PROCESSING_AGE, 300);
