...
/// Our view of what the genesis block should be. This will be used to cast the genesis vote
pub enum GenesisBlock {
    /// The block, ready to be voted on.
    Block(Block),
    /// Edge case: we produced the genesis block and have not yet shredded (and thus don't know the block_id) this block.
    /// This state represents a transitional state that must be upgraded to `GenesisBlock::Block` once shredding is complete
    /// before we cast the genesis vote.
    YetToShred(Slot),
}
...
pub struct GenesisVoteTracker {
    ...
    pub(crate) genesis_block: Option<GenesisBlock>,
...
impl MigrationStatus {
    ...
    pub fn post_migration_status() -> Self {
        let genesis_vote_tracker = GenesisVoteTracker {
            genesis_block: Some(GenesisBlock::Block((0, Hash::default()))),
            genesis_certificate: Some(CertificateMessage {
                certificate: Certificate::Genesis(0, Hash::default()),
        ...
        Self {
            ...
            genesis_wait: (Mutex::new(()), Condvar::new()),
            genesis_vote_tracker: RwLock::new(genesis_vote_tracker),
        }
    }
    ...
    pub fn notify_feature_flag(&self, slot: Slot) {
    ...
    pub fn genesis_block(&self) -> Option<Block> {
        ...
        Some(
            self.genesis_vote_tracker
                .read()
                .unwrap()
                .genesis_block
                .expect("If alpenglow is enabled there must be a genesis")
                .to_block()
                .expect("If alpenglow is enabled we must have the genesis block"),
        )
    }

    /// The block that is eligble to be the genesis block, which we cast our genesis vote for.
    /// Returns `None` if the migration is already complete or we have not yet received an eligble block.
    pub fn eligble_genesis_block(&self) -> Option<GenesisBlock> {
        if self.is_alpenglow_enabled() {
            return None;
        }
        self.genesis_vote_tracker.read().unwrap().genesis_block
    }
    ...
    pub fn is_genesis_certified(&self) -> bool {
        ...
        let Some(g_block) = genesis_tracker_r.genesis_block.and_then(|gb| gb.to_block()) else {
            return false;
        };
    ...
    pub fn set_genesis_block(&self, genesis: GenesisBlock) {
        ...
        match genesis_tracker_w.genesis_block.replace(genesis) {
            Some(GenesisBlock::Block(prev)) => panic!(
                "{} Attempting to overwrite genesis block {genesis:?} from {prev:?}. Programmer \
                 error",
                self.my_pubkey
            ),
            Some(GenesisBlock::YetToShred(slot)) => warn!(
                "{}: Block {slot} that we produced has finished shredding and is eligble as the \
                 genesis block. Casting our vote {genesis:?}",
                self.my_pubkey
            ),
                ...
                if genesis != (slot, block_id) {
                    error!(
                        "{}: We cast a genesis vote on {genesis:?}, however we have received a \
                         genesis certificate for ({slot}, {block_id}). This indicates two \
                         possibilities:
