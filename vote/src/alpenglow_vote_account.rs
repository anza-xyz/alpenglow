use {
    alpenglow_vote::{solana_program::program_error::ProgramError, state::VoteState},
    itertools::Itertools,
    serde::{
...
pub struct VoteAccount {
    // We can use spl_pod::bytemuck::pod_from_bytes to instantly convert account.data() to
    // VoteState with zero copy, so we don't need to save vote_state separately any more.
    // Of course, we should still verify account.data() can be succcessfully deserialized
    // when doing try_from.
...
pub type VoteAccountsHashMap = HashMap<Pubkey, (Stake, VoteAccount)>;
#[cfg_attr(feature = "frozen-abi", derive(AbiExample))]
    ...
    staked_nodes: OnceLock<
        Arc<
            HashMap<
                ...
                Stake,  // Total stake across all vote-accounts.
            >,
        >,
    >,
}
...
impl VoteAccount {
    ...
    pub fn node_pubkey(&self) -> &Pubkey {
        let vote_state = VoteState::deserialize(self.account.data())
            .expect("All errors should be handled on init");
        vote_state.node_pubkey()
    }

    #[cfg(test)]
    pub fn vote_state(&self) -> &VoteState {
        VoteState::deserialize(self.account.data()).expect("All errors should be handled on init")
    }
}
...
impl VoteAccounts {
    ...
    pub fn staked_nodes(&self) -> Arc<HashMap<Pubkey, Stake>> {
        self.staked_nodes
            .get_or_init(|| {
                Arc::new(
                    self.vote_accounts
...
impl TryFrom<AccountSharedData> for VoteAccount {
    ...
    fn try_from(account: AccountSharedData) -> Result<Self, Self::Error> {
        ...
        // We should keep the following deserialize to make sure account.data()
        // can be successfully deserialized.
        let _ = *VoteState::deserialize(account.data())?;
        Ok(VoteAccount { account })
    }
}
...
mod tests {
    use {
        super::*, bincode::Options, rand::Rng, solana_account::WritableAccount,
        solana_pubkey::Pubkey, std::iter::repeat_with,
    };
    ...
    fn new_rand_vote_account<R: Rng>(
        ...
        let alpenglow_vote_state = VoteState::new_for_tests(
    ...
    fn test_vote_account_try_from() {
        ...
        let (account, alpenglow_vote_state) = new_rand_vote_account(&mut rng, None);
        let lamports = account.lamports();
        let vote_account = VoteAccount::try_from(account.clone()).unwrap();
        assert_eq!(lamports, vote_account.lamports());
        assert_eq!(alpenglow_vote_state, *vote_account.vote_state());
