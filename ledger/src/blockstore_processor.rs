...
fn process_next_slots(
    ...
    for next_slot in &meta.next_slots {
        ...
        if next_meta.is_full() {
            ...
            set_alpenglow_ticks(&next_bank, &MigrationStatus::default());
            trace!(
...
pub fn set_alpenglow_ticks(bank: &Bank, migration_status: &MigrationStatus) {
    // TODO(ashwin): REMOVE once we have cert in block header
    if bank.get_alpenglow_genesis().is_none()
        && bank.slot() <= migration_status.genesis_slot().unwrap_or(Slot::MAX)
    {
        // Poh Bank
        return;
    }
