...
use {
    ...
    solana_clock::{DEFAULT_TICKS_PER_SECOND, NUM_CONSECUTIVE_LEADER_SLOTS, Slot, UnixTimestamp},
    solana_entry::{
...
impl Blockstore {
    ...
    ///
    /// Returns `None` if the slot is missing or not yet full.
    pub fn get_double_merkle_root(&self, slot: Slot) -> Option<Hash> {
        ...
        self.compute_double_merkle_root(slot)
    }

    /// Computes and stores the double merkle root metadata for a full slot.
    fn compute_double_merkle_root(&self, slot: Slot) -> Option<Hash> {
        let slot_meta = self
            .meta_cf
            .get(slot)
            .expect("Blockstore operations must succeed")?;
        ...
        // SIMD-0317 guarantees fixed-size FEC sets for these shreds.
        ...
        let parent_meta = self
            ...
            .get(slot)
            .expect("Blockstore operations must succeed")
            .expect("Slot cannot be full without parent metadata");
        ...
        let merkle_tree_leaves: Vec<std::result::Result<Hash, shred::Error>> = self
            .merkle_root_meta_cf
            .multi_get_bytes(&keys)
            .map(|get_result| {
                let bytes = get_result
                    .expect("Blockstore operations must succeed")
                    .expect("Merkle root metadata must exist for every FEC set in a full slot");
                let merkle_root = bincode::deserialize::<MerkleRootMeta>(bytes.as_ref())
                    .expect("Merkle root metadata should always deserialize")
                    .merkle_root()
                    .expect("Merkle root should be present for post-SIMD-0317 shreds");
                Ok(merkle_root)
            })
    ...
    fn commit_updates_to_write_batch(
        ...
        for (&slot, working_parent_meta) in &shred_insertion_tracker.parent_metas {
            if !working_parent_meta.should_write() {
                // No need to rewrite the column
                continue;
            }
            let parent_meta = working_parent_meta.as_ref();

            if parent_meta.populated_from_update_parent() {
                update_parent_signals.push(UpdateParentSignal {
                    slot,
                    parent_slot: parent_meta.parent_slot,
                    parent_block_id: parent_meta.parent_block_id,
                    replay_fec_set_index: parent_meta.replay_fec_set_index,
                });
            }
    ...
    fn should_write_parent_meta(slot: Slot, new: &ParentMeta, prev: &ParentMeta) -> Result<bool> {
        ...
        if !slot.is_multiple_of(NUM_CONSECUTIVE_LEADER_SLOTS) {
    ...
    pub fn get_data_shreds_for_slot_from_location(
        ...
        if matches!(location, BlockLocation::Alternate { .. }) {
            return Ok(Vec::new());
        }
        ...
        let shred_bytes_iter: Box<dyn Iterator<Item = Box<[u8]>>> = match location {
            ...
            BlockLocation::Alternate { .. } => Box::new(std::iter::empty()),
        };
